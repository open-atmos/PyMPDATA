...
def __make_antidiff(*, atv, ats, non_unit_g_factor, options, n_dims, last_pass):
    ...
    def antidiff_basic(psi, g_c, _):
        ...
        tmp = a_term(psi)
        result = (np.abs(atv(*g_c, .5)) - atv(*g_c, +.5) ** 2) * tmp
    ...
    def antidiff_variants(psi, g_c, g_factor):
        ...
        if third_order_terms:
            ...
            tmp = (
                3 * atv(*g_c, .5) * np.abs(atv(*g_c, .5)) / g_bar
                - 2 * atv(*g_c, .5) ** 3 / g_bar ** 2
                - atv(*g_c, .5)
            ) / 6
            ...
            if n_dims > 1:
                ...
                tmp = tmp / (2 * g_bar) * (
                    np.abs(atv(*g_c, .5)) - 2 * atv(*g_c, .5) ** 2 / g_bar
                )
        ...
        if divergent_flow:
            ...
            tmp = -.25 * atv(*g_c, .5) * (atv(*g_c, 1.5) - atv(*g_c, -.5))
