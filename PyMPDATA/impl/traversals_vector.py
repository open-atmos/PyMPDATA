...
def _make_apply_vector(*, indexers, jit_flags, halo, n_dims, n_threads,
    ...
    def apply_vector_impl(thread_id, out_meta,
        ...
        for i in range(rng_outer[RNG_START] + halos[OUTER][OUTER],
                       rng_outer[RNG_STOP] + halos[OUTER][OUTER] + (ONE_FOR_STAGGERED_GRID if last_thread else 0)
                       ) if n_dims > 1 else (INVALID_INDEX,):
            for j in range(rng_mid3d[RNG_START] + halos[MID3D][MID3D],
                           rng_mid3d[RNG_STOP] + ONE_FOR_STAGGERED_GRID + halos[MID3D][MID3D]
                           ) if n_dims > 2 else (INVALID_INDEX,):
                for k in range(rng_inner[RNG_START] + halos[INNER][INNER],
...
def _make_fill_halos_vector(*, indexers, jit_flags, halo, n_dims, chunker, spanner):
    ...
    def outer_outer(span, comp, fun, first_thread, last_thread):
        if first_thread:
            for i in range(halos[OUTER][OUTER] - 1,
                           -1, -1):  # note: non-reverse order assumed in Extrapolated
                for j in range(0,
    ...
    def outer_mid3d(span, comp, fun, rng_outer, last_thread):
        for i in range(rng_outer[RNG_START],
                       rng_outer[RNG_STOP] + ((ONE_FOR_STAGGERED_GRID + 2 * halos[OUTER][OUTER]) if last_thread else 0)):
            for j in range(0,
                           halos[OUTER][MID3D]):
                for k in range(0,
                               span[INNER] + 2 * halo):
