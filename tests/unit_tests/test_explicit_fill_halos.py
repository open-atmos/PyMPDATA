...
def test_explicit_fill_halos(field_factory, halo, bc, n_threads):
    ...
    traversals = Traversals(grid=field.grid, halo=halo, jit_flags=JIT_FLAGS, n_threads=n_threads, left_first=tuple([True]*MAX_DIM_NUM),buffer_size=0)
    ...
    #act
    field._fill_halos(traversals) #pylint:disable=protected...
