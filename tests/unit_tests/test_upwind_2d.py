...
@pytest.mark.parametrize(
    "shape, ij0, out, courant_number",
    [
        pytest.param((3, 1), (1, 0), np.array([[0.0], [0.0], [44.0]]), (1.0, 0.0)),
        pytest.param((1, 3), (0, 1), np.array([[0.0, 0.0, 44.0]]), (0.0, 1.0)),
        pytest.param((1, 3), (0, 1), np.array([[44.0, 0.0, 0.0]]), (0.0, -1.0)),
        pytest.param(
            (3, 3),
            (1, 1),
            np.array([[0, 0, 0], [0, 0, 22], [0.0, 22.0, 0.0]]),
            (0.5, 0.5),
        ),
        pytest.param(
            (3, 3),
...
def test_upwind(shape, ij0, out, courant_number):
    ...
    advectee = ScalarField(
        scalar_field_init, halo=options.n_halo, boundary_conditions=bcs
    )
    advector = VectorField(
        vector_field_init, halo=options.n_halo, boundary_conditions=bcs
    )
