...
from examples.PyMPDATA_examples.burgers_equation.burgers_equation import (
...
class TestGeneralNumericalVerification(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.states, self.x, self.dt, self.dx = run_numerical_simulation(
            nt=400, nx=100, t_max=1 / np.pi
        )
    ...
    def test_solution_within_bounds(self):
        ...
        self.assertGreaterEqual(
            min_val, -1.0, f"Minimum value {min_val} is less than -1."
        )
        self.assertLessEqual(
            max_val, 1.0, f"Maximum value {max_val} is greater than 1."
        )
