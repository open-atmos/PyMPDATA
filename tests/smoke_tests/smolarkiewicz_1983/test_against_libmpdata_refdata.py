...
STATS = {
    ...
    Options(n_iters=2, infinite_gauge=True, nonoscillatory=True): {
        0: {"min(solution)": 0.00000000, "max(solution)": 4.00000000},
        556: {
...
@pytest.mark.parametrize(
    "options", tuple(pytest.param(opt, id=str(opt)) for opt in STATS)
)
def test_against_libmpdata_refdata(options):
    ...
    for timesteps in STATS[options]:
        ...
        if steps_done > 0:
            ...
            actual[steps_done]["L2"] = (
                1 / time * (1 / volume * np.sum((absdiff) ** 2)) ** 0.5
            )
