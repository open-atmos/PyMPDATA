...
def test_size_distribution(grid, coord, plot=False):
    ...
    if plot:
        ...
        pyplot.plot(
            numpdfx, pdf_t(numpdfx.magnitude) * n_unit, label="pdf", linestyle="--"
        )
        pyplot.legend()
...
def test_quad_vs_midpoint():
    ...
    a = discretised_analytical_solution(
        x, np.sin, midpoint_value=True, r=x[:-1] + np.diff(x) / 2
    )
    b = discretised_analytical_solution(x, np.sin, midpoint_value=False)
    test_val = np.abs((a - b) / a)
    # Assert
    assert (test_val > 0.05).all()
    assert (test_val < 0.1).all()
