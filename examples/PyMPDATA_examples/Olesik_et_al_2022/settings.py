import numpy as np
import pint
from PyMPDATA_examples.Olesik_et_al_2022 import (
    East_and_Marshall_1954,
    equilibrium_drop_growth,
)
...
class Settings:
    def __init__(
        ...
        self.C = (1 * si.gram / si.kilogram) / self.mixing_ratio(
            self.size_distribution.pdf
        )
        ...
        for mr in self.mixing_ratios:
            ...
            def findroot(ti, mr=mr, t_unit=t_unit):
                return (
                    mr
                    - self.mixing_ratio(
                        equilibrium_drop_growth.PdfEvolver(
                            self.pdf, self.drdt, ti * t_unit
                        )
                    )
                ).magnitude
    ...
    def mixing_ratio(self, pdf):
        ...
        I = (
            integrate.quad(pdfarg, r_min.to(xunit).magnitude, np.inf)[0]
            * yunit
            * xunit**4
        )
        return (I * 4 / 3 * np.pi * self.rho_w / self.rho_a).to(
            self.si.gram / self.si.kilogram
        )
