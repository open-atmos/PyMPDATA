    ...
    *,
    ...
    for coord, case in cases.items():
        ...
        for t in [case.dt * nt for nt in case.out_steps]:
            ...
            analytical.append(
                discretised_analytical_solution(
                    rh.magnitude,
    ...
    for coord, case in cases.items():
        ...
        for opts in output[coord]["numerical"]:
            ...
            error_L2[opts] = L2(
                numerical[-1].magnitude, analytical[-1].magnitude, case.out_steps[-1]
            )
...
class Result(
    namedtuple("Result", ("dt", "out_steps", "grid_layout_str", "option_str", "result"))
):
    __slots__ = ()
...
def Case(result: Result):
    return namedtuple("Case", ("dt", "out_steps", "grid_layout_str"))(
        dt=result.dt, out_steps=result.out_steps, grid_layout_str=result.grid_layout_str
    )
...
def rel_disp(r, psi, psi_coord):
    ...
    for i, psi_i in enumerate(psi):
        ...
        A_i = psi_coord.moment_of_r_integral(
            psi_coord.x(r[i + 1]), 1
        ) - psi_coord.moment_of_r_integral(psi_coord.x(r[i]), 1)
        B_i = psi_coord.moment_of_r_integral(
            psi_coord.x(r[i + 1]), 2
...
def third_moment(r, psi, psi_coord, normalised=True):
    ...
    for i, psi_i in enumerate(psi):
        ...
        integral_i = psi_coord.moment_of_r_integral(
            psi_coord.x(r[i + 1]), 3
