...
def plot_output(times, output, settings, return_data=True):
    lambdas_analytic = formulae.d2_el_lamb_lamb_t_evol(
        times=[0] + list(times), lamb_x0=settings.lx0, lamb_y0=settings.ly0
    )[1:]

    data = {}
    cuts = ("x", "y")
    _, axs = pylab.subplots(
        nrows=len(times), ncols=len(cuts), figsize=(9, 9), constrained_layout=True
    )
    momenta = {"x": "uh", "y": "vh"}
    for i_cut, cut in enumerate(cuts):
        if cut == "x":
            ...
            coord = settings.dx * (np.linspace(-n // 2, n // 2, n) + 0.5)
            x = coord
            y = 0
        else:
            n = settings.ny
            idx = (slice(n // 2, n // 2 + 1), slice(None, None))
            coord = settings.dy * (np.linspace(-n // 2, n // 2, n) + 0.5)
            x = 0
            y = coord
        for i_t, t in enumerate(times):
            ...
            datum = data[key]
            datum["h_analytic"] = formulae.amplitude(
                x, y, *lambdas_analytic[i_t, (0, 2)]
            )
            where = "mid"
            ...
            np.divide(
                output[t][momenta[cut]][idx].squeeze(),
                datum["h_numeric"],
                where=mask,
                out=datum["q_h_numeric"],
            )
            twin.step(
                datum["coord"],
                datum["q_h_numeric"],
            ...
            datum["q_h_analytic"] = np.where(
                ...
                datum["coord"]
                / lambdas_analytic[i_t, 0 + 2 * i_cut]
            ...
            axs[i_t, i_cut].set_ylim(-0.6, 0.6)
            axs[i_t, i_cut].set_yticks((-0.5, 0, 0.5))
