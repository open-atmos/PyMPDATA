...
from MPyDATA.fields.factories import clone_scalar_field, clone_vector_field, make_scalar_field, make_vector_field
from MPyDATA.fields.math import div
...
class MPDATAFactory:
    ...
    @staticmethod
    def uniform_C_1d(psi: IScalarField, C: IVectorField, opts: Options):
        nx = psi.shape[0]
        halo = MPDATAFactory.n_halo(opts)

        state = make_scalar_field(psi, halo)
        GC = make_vector_field(data=[np.full((nx + 1,), C)], halo=halo)
        g_factor = make_scalar_field(np.ones((nx,)), halo=0)  # TODO
        return MPDATAFactory._mpdata(state=state, GC_field=GC, g_factor=g_factor, opts=opts)

    @staticmethod
    def uniform_C_2d(psi: IScalarField, C: IVectorField, opts: Options):
        nx = psi.shape[0]
        ny = psi.shape[1]
        halo = MPDATAFactory.n_halo(opts)

        state = make_scalar_field(psi, halo)
        GC = make_vector_field(data=[
            np.full((nx + 1, ny), C[0]),
            np.full((nx, ny+1), C[1])
        ], halo=halo)
        g_factor = make_scalar_field(np.ones((nx,ny)), halo=0)  # TODO
        return MPDATAFactory._mpdata(state=state, GC_field=GC, g_factor=g_factor, opts=opts)
    ...
    def kinematic_2d(grid, size, dt, stream_function: callable, field_values: dict, g_factor: np.ndarray, opts):
        ...
        G = make_scalar_field(g_factor, halo=0)
    ...
    def _mpdata(
            state: IScalarField,
            g_factor: IScalarField,
            GC_field: IVectorField,
            opts: Options
    ):
        # TODO: move to tests
        if len(state.shape) == 2:
            assert state._data.shape[0] == GC_field._data_0.shape[0] + 1
            assert state._data.shape[1] == GC_field._data_0.shape[1]
            assert GC_field._data_0.shape[0] == GC_field._data_1.shape[0] - 1
            assert GC_field._data_0.shape[1] == GC_field._data_1.shape[1] + 1
        # TODO: assert G.data.shape == state.data.shape (but halo...)
        # TODO assert halo

        prev = clone_scalar_field(state)  # TODO rename?
        GC_antidiff = clone_vector_field(GC_field)
        flux = clone_vector_field(GC_field)
        halo = state.halo

        if (opts.n_iters != 1) & opts.fct:
            psi_min = clone_scalar_field(state)
            psi_max = clone_scalar_field(state)
            beta_up = clone_scalar_field(state)
            beta_dn = clone_scalar_field(state)
        else:
            psi_min = None
            psi_max = None
            beta_up = None
...
def _uniform_scalar_field(grid, value: float, halo: int):
    ...
    return make_scalar_field(data=data, halo=halo)
...
def _nondivergent_vector_field_2d(grid, size, halo, dt, stream_function: callable):
    ...
    result = make_vector_field(data=GC, halo=halo)
